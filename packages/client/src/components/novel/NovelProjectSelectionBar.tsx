...
type NovelProjectSelectionBarProps = {
...
// 渲染短剧项目多选底部操作栏
export function NovelProjectSelectionBar({
    selectedCount,
    deleting = false,
    onClearSelection,
    onDelete,
}: NovelProjectSelectionBarProps) {
    if (selectedCount <= 0) {
        return null;
    }
    ...
    return (
        <div className="pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-4">
            <div className="pointer-events-auto flex items-center gap-3 rounded-full bg-black px-5 py-3 shadow-[0_12px_40px_rgba(15,23,42,0.28)]">
                <span className="text-sm text-white">已选择 {selectedCount} 个项目</span>
                <button
                    type="button"
                    disabled={deleting}
